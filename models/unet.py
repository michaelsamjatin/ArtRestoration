...
# Shallow, basic UNet without Attention (Baseline model)
...
class ShallowUNet(nn.Module):
    """
    Simple UNet model with two double convolutions and not attention.
    """
    ...
    def forward(self, x):
        ...
        # Decoder path.
        x4 = self.upconv1(x3)
        x4 = torch.cat([enc2, x4], dim=1) 
        x4 = self.decoder1(x4)

        x5 = self.upconv2(x4) 
        x5 = torch.cat([enc1, x5], dim=1) 
        x5 = self.decoder2(x5) 
